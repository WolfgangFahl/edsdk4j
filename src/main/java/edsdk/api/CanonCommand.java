...
public abstract class CanonCommand<T> implements EdsObjectEventHandler {
   ...
   * @param camera
   */
  public void setCamera(final BaseCanonCamera camera) {
    this.camera = camera;
   ...
   * @return
   */
  public EdsError sendCommand(final EdsCameraCommand command, final long params) {
    return CanonUtils.toEdsError(CanonCamera.EDSDK.EdsSendCommand(camera
        .getEdsCamera(), new NativeLong(command.value()),
        new NativeLong(params)));
  }
  ...
  public EdsError sendStatusCommand(final EdsCameraStatusCommand command,
      final long params) {
    return CanonUtils.toEdsError(CanonCamera.EDSDK.EdsSendStatusCommand(camera
        .getEdsCamera(), new NativeLong(command.value()),
        new NativeLong(params)));
  }
   ...
   */
  @Override
  public NativeLong apply(NativeLong inEvent, Pointer inRef, EdsVoid inContext) {
    EdsObjectEvent lInEvent = EdsObjectEvent.enumOfValue(inEvent.intValue());
    EdsError lResult = apply(lInEvent, inRef, inContext);
    return new NativeLong(lResult.value());
  }
   ...
   */
  public EdsError apply(final EdsObjectEvent inEvent, final Pointer inRef,
      EdsVoid inContext) {
  ...
  public T get() {
    try {
      ...
      // this shouldn't happen since get( 0 ) internally handles
      // InterruptedException when timeout=0.
      e.printStackTrace();
    }
    return null;
  }
  ...
  public T get(final long timeout) throws InterruptedException {
    if (this.camera == null) {
      ...
      System.err
          .println("  This way you might wait forever until .get() returns. ");
    }
    ...
    if (finished()) {
      ...
      throw new InterruptedException(
          "edsdkp5 - command didn't return the result in time");
    }
  }
